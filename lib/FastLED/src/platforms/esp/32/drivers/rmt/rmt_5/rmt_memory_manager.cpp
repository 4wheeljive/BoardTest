...
Result<size_t, RmtMemoryError> RmtMemoryManager::allocateTx(uint8_t channel_id, bool use_dma, bool networkActive,
    ...
    // Calculate adaptive buffer size based on network state (unless overridden)
    size_t default_blocks = calculateMemoryBlocks(networkActive);
    size_t mem_blocks = mem_blocks_override ? mem_blocks_override : default_blocks;
    if (mem_blocks == 0) {
        FL_WARN("RMT TX allocation failed: mem_blocks_override was 0");
    ...
    FL_LOG_RMT("RMT TX channel " << static_cast<int>(channel_id)
               ...
               << ((mem_blocks_override && mem_blocks != default_blocks) ? ", override" : "")
