...
static esp_err_t rmt_cache_sync(void* buffer_ptr, size_t buffer_size) {
    static bool s_cache_msync_supported = true;

    if (!buffer_ptr || buffer_size == 0) {
        return ESP_OK;
    }
    if (!s_cache_msync_supported) {
        return ESP_OK;
    }
    ...
    // Non-cacheable DMA memory can return ESP_ERR_INVALID_ARG - that's OK
    if (err == ESP_ERR_INVALID_ARG) {
        s_cache_msync_supported = false;
        return ESP_OK;
    }
...
class ChannelEngineRMTImpl : public ChannelEngineRMT {
  public:
    ChannelEngineRMTImpl()
        ...
        // Suppress esp_cache_msync() error logs for non-cacheable DMA buffers
  ...
  private:
    ...
    bool createChannel(ChannelState *state, gpio_num_t pin,
        ...
        if (tryDMA && dataSize > 0) {
            ...
            // for safety)
            constexpr fl::size kMaxDmaSymbols = 1024;
            constexpr fl::size kMinDmaSymbols = 64;

            fl::size dma_mem_block_symbols = (dataSize * 8) + 16;
            if (dma_mem_block_symbols & 1) {
                dma_mem_block_symbols++;
            }

            fl::size start_symbols = dma_mem_block_symbols;
            if (start_symbols > kMaxDmaSymbols) {
                start_symbols = kMaxDmaSymbols;
            }

            FL_LOG_RMT("DMA allocation request: "
            ...
            esp_err_t dma_err = ESP_FAIL;
            fl::size last_tried = 0;
            const fl::size candidates[] = { start_symbols, 512, 256, 128, kMinDmaSymbols };
            for (fl::size symbols : candidates) {
                if (symbols == 0 || symbols > start_symbols || symbols == last_tried) {
                    continue;
                }
                if (symbols & 1) {
                    symbols++;
                }
                dma_config.mem_block_symbols = symbols;
                dma_err = rmt_new_tx_channel(&dma_config, &state->channel);
                if (dma_err == ESP_OK) {
        ...
        size_t block_words = SOC_RMT_MEM_WORDS_PER_CHANNEL;
        size_t requested_blocks = RmtMemoryManager::calculateMemoryBlocks(networkActive);
        ...
        if (mMemoryReductionOffset >= block_words) {
            size_t reduction_blocks = mMemoryReductionOffset / block_words;
            if (reduction_blocks > 0) {
                if (requested_blocks > reduction_blocks) {
                    FL_LOG_RMT("Applying memory reduction offset: " << mMemoryReductionOffset
                               << " symbols (" << reduction_blocks << " blocks)");
                    requested_blocks -= reduction_blocks;
                } else {
                    requested_blocks = 1;
                }
            }
...
#if FASTLED_RMT5_DMA_SUPPORTED && defined(CONFIG_IDF_TARGET_ESP32S3)
        // DMA on ESP32-S3 effectively consumes one TX memory block
        if (mDMAChannelsInUse > 0) {
            size_t effective_available = memMgr.availableTxWords();
            if (effective_available >= block_words) {
                effective_available -= block_words;
            } else {
                effective_available = 0;
            }
            size_t max_blocks = effective_available / block_words;
            if (max_blocks == 0) {
                FL_WARN("Memory manager TX allocation failed: no TX blocks available after DMA reservation");
                return false;
            }
            if (requested_blocks > max_blocks) {
                FL_LOG_RMT("DMA in use: reducing TX buffer blocks from "
                           << requested_blocks << " to " << max_blocks);
        ...
        auto alloc_result = memMgr.allocateTx(state->memoryChannelId, false,
                                              networkActive, requested_blocks); // false = non-DMA
        if (!alloc_result.ok()) {
        ...
        size_t allocated_blocks = block_words ? (mem_block_symbols / block_words) : 0;
        ...
        if (memMgr.getDMAChannelsInUse() > 0) {
            FL_LOG_RMT("✓ TX Channel #"
                       << channel_num
                       << ": Non-DMA (" << allocated_blocks << " blocks, "
                       << mem_block_symbols << " words) - DMA slot taken by another channel");
        } else {
            FL_LOG_RMT("✓ TX Channel #"
                       << channel_num
                       << ": Non-DMA (" << allocated_blocks << " blocks, "
                       << mem_block_symbols << " words) - No DMA support on platform");
        }
        ...
        if (err == ESP_ERR_NOT_FOUND && mem_block_symbols > SOC_RMT_MEM_WORDS_PER_CHANNEL) {
            ...
            const size_t block_words = SOC_RMT_MEM_WORDS_PER_CHANNEL;
            const size_t original_blocks = (block_words > 0)
                ? ((original_symbols + block_words - 1) / block_words)
                : 0;
            const size_t min_blocks = 1;
            bool recovery_succeeded = false;

            // Progressively reduce memory by whole blocks until it works or we hit minimum
            for (size_t reduced_blocks = original_blocks; reduced_blocks > min_blocks; --reduced_blocks) {
                size_t reduced_symbols = (reduced_blocks - 1) * block_words;
                retry_count++;
                ...
                if (err == ESP_OK) {
                    ...
                    memMgr.free(state->memoryChannelId, true);
                    auto reduced_alloc = memMgr.allocateTx(state->memoryChannelId, false,
                                                           networkActive,
                                                           (reduced_symbols / block_words));
                    if (!reduced_alloc.ok()) {
                        FL_WARN("Memory manager re-allocation failed after recovery; accounting may be off");
                    }
            ...
            if (!recovery_succeeded) {
                ...
                msg << "\n========================================\n"
                    ...
                    << "  Minimum attempted: " << (min_blocks * block_words) << " symbols\n"
